...
def try_all_pieces():
    ...
    random.shuffle(bag)
    for piece in bag:
        random.shuffle(possible_piece_config_table[piece])
        for ori_idx, col_idx in possible_piece_config_table[piece]:
...
def play_a_map(mode, seed = None):
    ...
    record.board = []
    for i in range(10):
        if generate_a_ds_map(Setting.no_of_unreserved_piece) is True and mygame.get_max_height() < 17:
            break
        else:
            if i%2 == 1:
                generate_final_map()
